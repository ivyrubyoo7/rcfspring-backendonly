...
public class ITAssetService {
    ...
    public List<ITAsset> getAllAssets() {
        ...
        if ("ADMIN".equalsIgnoreCase(role)) {
            assets = itAssetRepository.findAll();
        }

        // 🔥 OFFICER / MANAGER
        else if ("OFFICER".equalsIgnoreCase(role) || "MANAGER".equalsIgnoreCase(role)) {

            // 🏢 CORPORATE (no plant)
            if (plantId == null) {
                assets = itAssetRepository.findByDepartment_Id(departmentId);
            }

            // 🏭 PLANT + DEPARTMENT
            else {
                assets = itAssetRepository.findByPlant_IdAndDepartment_Id(plantId, departmentId);
            }
        }
